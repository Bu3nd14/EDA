...
| Ultimo lotto chiuso | **L41b1** — il temporizzatore, l'hardware (**ADR-049**, scelte dell'utente approvando il piano: ATtiny3216 + MCP4822, firmware in `firmware/preamp_timer/` provato sull'host, L41b diviso in due). In `psu.py` al posto di J509: il micro, **Δ in hardware** (RC su `PERMIT_T` caricato da `MUTE_REQ` o `PERMIT_REQ`, comparatore; `MUTE_G` ≤ `PERMIT_G`), **l'interruttore che toglie `VRELAY` alla scheda audio in standby**, tenuto Δ₂ dopo il permissivo (un micro in reset non fa più il caso «senza Δ»), e il **pilota esponenziale delle LDR**. Simulato: Δ ≥ **16,9 ms** col micro in reset, a zero, in una perdita di rete o col guasto di U503; controfattuale 0; `VRELAY` a J1 = 0 V in standby, 92,5 mW dal secondario di T2; profilo v4 calibrato entro **±0,92 dB** da 15 a 60 °C; rumore all'anodo ~500× sotto il limite. 2j esteso e 2e `--timer` (ADR-022 condizione 1): 15 falsi + `main` falliscono. Deck V2 byte-identico, scheda audio non toccata. **NC-037** aperta col criterio numerico (T2 ≤ ~0,40 W a vuoto); **apre NC-038** (VTL5C4: 14,8 mA max a 58 °C contro i 20 mA del contratto di J3). Specifica in `firmware/preamp_timer/spec/timer_spec.md`. Report `reports/2026-09-26-L41b1-temporizzatore-hardware.md`. Prima: **L41a** — l'alimentatore: le decisioni dell'utente (**ADR-048**) e il sorgente `circuits/preamp/psu.py`, senza temporizzatore. Due trasformatori standard (toroidale 2×15 V 50 VA per i rail, piccolo toroidale da 12 V AC per `VRELAY` e lo standby), rail lineari TPS7A4701 / TPS7A3301, **nessuno switching**; `VRELAY` 12 V (scheda audio: LED 4,99 k, bobine 12 VDC; deck V2 byte-identico); standby con frontale a bassa tensione e IEC posteriore bipolare; sorvegliante in hardware sui rail (13,56 / −13,52 V), sulla rete (~14 ms) e su `VRELAY` (11 V). Simulato da solo: alla perdita di rete il jack si stacca coi rail a 15 V; tenuta dei rail 19,4 ms (≥ 16); `VRELAY` ≥ 25 ms. Controllo del cablaggio fra le schede (2j, 8 falsi). P5 a ~17 / ≤ 20 W (decisione dell'utente). **Apre NC-037** (standby > 0,5 W, rimedio in L41b). NC-036 resta aperta. Report `reports/2026-09-26-L41a-alimentatore-potenza-sorvegliante.md`. Prima: **L30** — lo spegnimento, il failsafe dell'alimentatore e il calore, con le decisioni dell'utente (**ADR-046**, **ADR-047**, **P9**). **Lo spegnimento è l'interruttore morbido**: il temporizzatore completa il mute, poi stacca il relè di rete (~7 s). V2 regge, 12 corse su 12, peggiore 2,7 µV (~+2 dB SPL di picco a 1 m). **La perdita di rete è un guasto**: sorvegliante sui rail a \|13,5 V\|, tenuta ≥ 1500 µF effettivi per rail, `VRELAY` tenuta ≥ 25 ms per l'ordine di ADR-045. Tetto di non-danno 0,87 V (~112 dB), obiettivo 2 mV (~60 dB), 12 corse su 12 ≤ 1,77 mV (~58 dB); senza Δ 69 mV (~90 dB). Il corto istantaneo di un rail è accettato (30 W contro 100 W). Regolazione persa col rail + a ~10,6 V (rimisurata). **Calore**: 15,4–17,8 W nel telaio, ≤ 55 °C anche in un vano chiuso stretto, P5 aggiornato. Il banco è generato sul sorgente (`--matrice l30`); deck V2 byte-identico, sorgente toccato solo nei commenti. **Chiude NC-028 e NC-029, apre NC-036** (l'alimentatore che realizza P9, bloccante per G2). Report `reports/2026-09-26-L30-spegnimento-failsafe-calore.md`. Prima: **L35** — comandi e LED a pannello nel sorgente, con **ADR-045** come primo punto. K6 su un comando proprio, **`PERMIT_CMD`**, rilasciato Δ (nominale 20 ms, ≥ 10 ms) dopo `MUTE_CMD` all'inserimento del mute: il residuo di L36, fino a ~90 dB SPL di picco a 1 m, torna a un cambio sotto mute, ~−22 dB, e lo stesso per il trim. **J4** porta `MUTE_CMD`, `PERMIT_CMD` e l'interruttore di mute **SW3** al temporizzatore, col contratto scritto accanto (i 13 ms di ADR-027 passano su `PERMIT_CMD`). I LED escono dalla scheda: **J5** trim, **J6** guadagno, **J7** il LED rosso di mute, letto da `VTRIM` (i due NC di K6) via **R3**. Il 2e prova comandi, finestra Δ e LED a pannello (quelli del trim per la prima volta); 14 falsi e `main` falliscono. ERC 47 come prima, deck V2 byte-identico, budget ~175 mA a 5 V (bobine 168,8 + LED ~6 in mute). **Chiude NC-032** e in NC-028 il residuo di L36. Nessuna ADR nuova. Report `reports/2026-09-25-L35-comandi-e-led-a-pannello.md`. Prima: **L36** — il guadagno interbloccato dal mute (ADR-041, strada B di ADR-030). In `circuits/preamp/gain_interlock.py`: K1/K5 al lato alto, gli ausiliari **K11/K12** in autoritenuta da `VHOLD` (NO del polo 1 di K6), **SW2** a 4 poli con un **polo ponte** per bobina, tre LED dello stato vero dai poli liberi degli ausiliari. **La corsa al rilascio è chiusa per struttura**: il datasheet del G6K non dà induttanza, rilascio minimo né trasferimento, quindi nessun tempo la chiude; il ponte alimenta la bobina accesa senza contatti di K6. Il 2e lo prova sulla netlist (trasferimento compreso; 11 falsi + `main` falliscono); simulato **0 cadute su 480 celle** contro **373** senza ponte. Deck V2 rigenerato byte-identico. Budget bobine **168,8 mA** a 5 V (+ vincolo Schottky). **Residuo** portato all'utente: manopola girata fuori mute e poi mute inserito, ordinato come il trim ma non chiuso dal datasheet (NC-028). Nessuna ADR nuova. Report `reports/2026-09-25-L36-guadagno-interbloccato.md`. Prima: **L29e** — la geometria iii nel sorgente (ADR-044). In `preamp_audio.py` ogni polo di K2–K4 è un **deviatore**: COM al lato del condensatore, NC a massa, NO al jack. Piedinatura riletta dal datasheet (pagina 6 del PDF). Il bleed lato condensatore resta, per decisione dell'utente («Tenerlo»). Il 2e asserisce la geometria (fatto fallire su `main` e su 4 falsi); il deck versionato si **genera dalla netlist** e rifiuta se non è la iii. Le celle peggiori di L29d2 sul sorgente: **18 verdetti su 18 uguali al banco, 0 fuori**: la parte del mute di NC-028 è **confermata**. Nessuna ADR nuova. Report `reports/2026-09-25-L29e-geometria-iii-sorgente.md`. Prima: **L29d2** — la matrice di L29c sulla geometria scelta dall'utente, **iii**: serie al jack più derivazione dal lato del condensatore. Contatto aperto 0,1 pF dal datasheet del G6K. **0 celle fuori su 253**, e 0 su 47 col cavo da 100 pF e i 10 kΩ. Cambio a relè chiuso da 111 a **0,17 µV**, dispersione da 267 a ≤ 0,60 µV, accensione da 11 mV a **4,4 µV**, musica invariata. **ADR-044**. L'utente ha chiuso **la parte del mute di NC-028**; resta lo spegnimento (L30). Il sorgente è di **L29e**. Report `reports/2026-09-25-L29d2-matrice-contatto-serie.md`. Prima: **L29d** — la sonda del contatto in serie al jack. Prima di correre, una previsione: con la serie aperta il lato condensatore resta sospeso, e un salto d'offset preso a mute inserito arriva al jack al rilascio. Confermata: serie sola (ii) e serie + derivazione al jack (iA) danno 10 mV dopo il cambio di guadagno e fino a 190 mV dopo l'accensione; iB 0,3–5,6 mV. **La serie con la derivazione dal lato del condensatore (iii) regge su tutto tranne un'accensione (205 µV, passaggio capacitivo dei 5 pF ipotizzati)**. Controfattuale entro 0,09 %. Nessuna ADR: sceglie l'utente (L29d2). NC-028 resta aperta. Report `reports/2026-09-25-L29d-sonda-contatto-serie.md`. Prima: **L29c** — il caso peggiore di V2 col mute reale. Con la musica regge ovunque (S ≤ 11 dB); criterio 3 di ADR-030 soddisfatto. **Fuori, senza musica**: il cambio di guadagno o trim a relè chiuso (fino a 267 µV con la dispersione), l'accensione e lo spegnimento, perché il contatto in derivazione attenua solo ~1/471. **ADR-043** (spegnimento e failsafe all'alimentatore). NC-028 resta aperta. Report `reports/2026-09-23-L29c-v2-caso-peggiore.md`. Prima: **L40** — V1 coi modelli del costruttore. Causa separata: la caduta è la **CJE dei MJE** (NC-025), non i MMBT. **ADR-042** (decisione dell'utente): Miller C124 da 470 pF a **1 nF**, corrente di riposo tenuta a **~20,3 mA** (R128 1,69 kΩ). V1 ≥ **62,08°** su ogni istanza e nel gruppo B; regressione di L39 rifatta, regge. Prezzo dichiarato: slew −1,68 V/µs, PSRR+ −6,5 dB (limiti per tono di ADR-020 ricalcolati), banda 912 kHz. **Chiude NC-034 e NC-035.** Report `reports/2026-09-23-L40-v1-costruttore.md`. Prima: **L39** |
| **Prossimo lotto** | **L41b2 IN CORSO (interrotto il 2026-09-26 per il limite d'uso, ramo `worktree-L41b2`, pushato)**. Fatto: NC-038 decisa dall'utente, **cima LDR 12 mA → ADR-050**; `firmware/preamp_timer/src/timer_core.c`; test sull'host `test/run_host_tests.sh` (65 + 45 controlli verdi, **21 falsi su 21**), agganciato come blocco 2k di `run_tests.sh` (non ancora eseguito lì); banco LDR a 12 mA entro +0,28 dB, E5 5,049 µV, E3 110,7 kΩ (`data/2026-09-26/L41b2/`); specifica aggiornata; SPI su PA2 verificata (hardware va bene). **Resta**: far convergere e analizzare i 7 casi SPICE (`data/2026-09-26/L41b2/deck/tutti_seq.sh`, poi `analizza_seq.py`; accensione, rilascio, inversione erano già a punto fisso, spegnimento/buco/guasto corretti e da rifare), NC-038 chiusa in NONCOMPLIANCE, report, README dei dati, limitations (#35: dopo una `tran` abortita `linearize`+`wrdata` scrive comunque un file lungo quanto la corsa), NEXT-SESSION, `chunk_close.sh L41b2`. Mandato originale: **L41b2 — il firmware del temporizzatore**: `timer_core.c` secondo `firmware/preamp_timer/spec/timer_spec.md` (accensione coi 13 ms, inserzione e rilascio reversibili, spegnimento morbido, debounce e OR con SW3, buco di rete nelle tre classi, ritenuta dopo un guasto, legge e calibrazione delle LDR), i test sull'host con `clang` fatti fallire su falsi, e la sequenza sul circuito col micro comportamentale pilotato dalle uscite del core. Prompt in `NEXT-SESSION.md`. Poi **L41c**, il banco di L30 col circuito vero (chiude NC-036) |
| Non conformità | **12 aperte, 2 bloccanti** (L41b1 apre NC-038) |
| Le bloccanti | **NC-004** (per G1: il rumore 1/f fuori dalla coppia d'ingresso) · **NC-036** (per G2: l'alimentatore che realizza P9, L41). NC-028 è chiusa da L30 (strada A, decisione dell'utente) |
| Suite | `run_tests.sh` **11 passed / 0 failed** a fine L41b1 (2026-09-26: il 2e con `--timer` su `psu.net`, il 2j esteso), eseguita dal worktree; `validate_models.py` 48/48 |
...
### L41b1 — il temporizzatore, l'hardware: Δ in hardware, lo standby, il pilota delle LDR (2026-09-26)

**ADR-049.** NC-036 resta aperta (L41b2, L41c); NC-037 aggiornata e aperta; **apre NC-038**.
Report: `reports/2026-09-26-L41b1-temporizzatore-hardware.md`. Dati: `data/2026-09-26/L41b1/`
(README). Specifica: `firmware/preamp_timer/spec/timer_spec.md`.

- **Le scelte dell'utente**: approvate col piano del lotto, senza commenti:
  - ATtiny3216 più un DAC MCP4822;
  - il firmware in `firmware/preamp_timer/`, con una specifica a tabella e i test sull'host;
  - L41b diviso in L41b1 (hardware) e L41b2 (firmware).

  Portate coi numeri: consumo, reperibilità, programmazione UPDI. I tre datasheet sono ora in
  `vendor/`.
- **Una conseguenza trovata leggendo, prima di scrivere**: il rimedio di NC-037 (togliere
  `VRELAY` alla scheda audio) con un interruttore comandato dal solo micro trasformava un reset
  del micro nel caso «senza Δ» (~90 dB SPL). L'interruttore è quindi tenuto in hardware, Δ₂
  dopo il permissivo.
- **Il sorgente** (`psu.py`):
  - Δ = RC su `PERMIT_T` caricato da `MUTE_REQ` **o** da `PERMIT_REQ`, comparatore U508 A;
  - D522: `MUTE_G` ≤ `PERMIT_G`;
  - Q505 su `VRELAY` (J1 pin 4 conserva il nome); sorvegliante, tenuta e K501 restano su
    `VRELAY_REG`;
  - due convertitori esponenziali (BCM847BS, MCP6004, specchio BCM857BS) su V5;
  - un pull-down su ogni richiesta; la rilettura di `MUTE_G` a 1 MΩ.

  ERC: 21 avvisi e 2 «errori», spiegati.
- **Simulato** (criteri scritti prima delle corse):
  - Δ 18,8 ms nominale e **16,9 ms** all'angolo minimo, col micro in reset, a zero, in reset
    durante una perdita di rete o un guasto di U503; il controfattuale senza C528 dà 0;
  - `VRELAY` a J1 sopra l'80 % fino a dopo `PERMIT_CMD`;
  - un firmware che abbassa solo `PERMIT_REQ` non muove niente;
  - al rilascio `PERMIT_CMD` arriva 0,04 ms prima di `MUTE_CMD`;
  - **standby: 0 V a J1, 92,5 mW dal secondario di T2**;
  - LDR: senza compensazione fino a +8,7 dB a 60 °C; compensate ±0,3 dB tranne la cima
    (−3 dB, ohmica); **calibrate a 20 / 2 mA entro ±0,92 dB**, anche coi LED a 2,0 V e V5 a
    4,90 V; DAC in reset 2–9 nA;
  - rumore all'anodo ~1,4 µV/√Hz contro 690.
- **I controlli**: il 2j (`check_psu_harness.py`) verifica l'interruttore, il micro e le
  sorgenti di J3. Il 2e `--timer` (`check_relay_safe_state.py`, ADR-022 condizione 1) asserisce
  sulla netlist:
  - pull-down;
  - ritardi a comparatore su RC;
  - nessuna scorciatoia verso i gate ritardati;
  - `MUTE_G` ≤ `PERMIT_G`;
  - la catena delle cariche.

  15 falsi su 15 e `main` falliscono. Il 2e ha trovato due difetti veri: `MAINS_REQ` senza
  pull-down, e la rilettura di `MUTE_G` a 47 kΩ.
- **Trappole**:
  - il «transient op» di ripiego in un `dc`, con un condensatore lento non assestato, dava un
    falso aggancio a 25 pA (limitations #33, aggiornata);
  - `%g` scriveva TE + 1 µs come «2», e il PWL del reset del micro non scendeva mai.
- **Aperti**:
  - **NC-037**: la causa è tolta; si chiude con T2 ≤ ~0,40 W a vuoto (giro BOM) e la misura;
  - **NC-038**: la VTL5C4 declassa il LED (40 mA − 0,9 mA/°C sopra 30 °C): 14,8 mA a 58 °C,
    mentre il contratto di J3 ne chiede 20. Decisione dell'utente con una ADR;
  - l'autoriscaldamento di Q2 a 20 mA non è modellato: resta al prototipo, e la calibrazione è
    il rimedio.

### L41a — l'alimentatore: decisioni, potenza, `VRELAY`, relè di rete, sorvegliante (2026-09-26)

**ADR-048.** NC-036 resta aperta (L41b, L41c); NC-011 col rimedio scelto; **apre NC-037**. Report:
`reports/2026-09-26-L41a-alimentatore-potenza-sorvegliante.md`. Dati: `data/2026-09-26/L41a/`
(README).

- **Le decisioni dell'utente, una alla volta**:
  - «2×15 V 50 VA standard»;
  - «non accetto uno switching, siamo troppo deboli sulla PSRR», poi «avere due trafo
    standard?», «si confermo»: `VRELAY` 12 V da un piccolo trasformatore suo;
  - «mi piace la 3, ma é possibile avere un secondo switch sul retro…?», «si confermo la 3 con
    l'interruttore posteriore»: lo standby, che supera una frase di ADR-046;
  - sul sorvegliante «accetto la tua raccomandazione»: comparatore, rivelatore di rete,
    ritenuta dopo un guasto;
  - «accetto l'ibrido»: il micro più l'ordine in hardware;
  - P5 a ≤ 20 W, «si lo accetto».
- **Due mie cifre sbagliate, corrette davanti all'utente prima della decisione**: 2×13 V non
  regge la rete bassa (simulato: −0,1 V); il calore non tornava dentro P5 (20,2 W).
- **Il sorgente** `circuits/preamp/psu.py`, secondo PCB: rete (IEC posteriore, F501, K501 sul
  solo toroidale), rail TPS7A4701 / TPS7A3301 con 2200 µF di tenuta, `VRELAY` con 4700 µF di
  serbatoio e 2200 µF di tenuta, stella NT501, sorvegliante in hardware sul gate del sink di
  `MUTE_CMD`, segnaposto J509 per L41b. ANY-OUT e V_REF letti sui datasheet.
- **Sulla scheda audio** solo valori fuori dal segnale (LED 4,99 k; bobine 12 VDC nei
  commenti): ERC 47, deck V2 byte-identico, 2e verde.
- **Simulato da solo** (regolatori e comparatore comportamentali, dichiarati):
  - perdita di rete: jack staccato in 14,2–14,4 ms **coi rail ancora a 15 V**;
  - un regolatore che cede: scatto a 13,56 / −13,52 V, tenuta 19,4 ms;
  - `VRELAY` 62,8 ms in tolleranza a rete −10 %.
- **Quattro difetti trovati prima di dare un numero**:
  - il pad del TPS7A3301 su IN (un corto a massa del grezzo), dal datasheet;
  - `VRELAY` non sorvegliata: il guasto del suo regolatore faceva cadere tutte le bobine
    insieme. Corretto col canale libero del comparatore e C523;
  - la tenuta di `VRELAY` a 14 ms con 2200 µF: 4700 µF;
  - gli `alter` che sopravvivono a `destroy all` (**limitations #34**).
- **Aperti**:
  - **NC-037**: in standby le bobine del trim sono pilotate di continuo, ~0,44 W da sole,
    contro 0,5 W. Rimedio: L41b toglie `VRELAY` alla scheda audio;
  - il corto dell'uscita di U503 = «senza Δ», ~90 dB SPL, da decidere in L41c;
  - la densità di rumore del TPS7A4701 contro ADR-020 (NC-011);
  - il testo del Reg. (UE) 2023/826 su EUR-Lex, non raggiungibile.
- **Modelli del costruttore** in `vendor/`: TPS7A3301 funziona in ngspice; TPS7A4701 carica ma
  sbaglia il punto di lavoro; TLV1701 non commuta. Nessuno in `models/`.
- **Trappole**:
  - un `alter` resta sul circuito dopo `destroy all` (#34);
  - SKiDL rinomina `GND` se una net la fonde (la presa centrale): il 2j l'ha preso;
  - SKiDL 2.3 non ha `NC`.

### L30 — lo spegnimento, il failsafe dell'alimentatore e il calore (2026-09-26)

**ADR-046**, **ADR-047**, **P9** nuovo, P5 aggiornato, V2 precisato. **Chiude NC-028** (strada
A) **e NC-029**; **apre NC-036**, bloccante per G2. Report:
`reports/2026-09-26-L30-spegnimento-failsafe-calore.md`. Dati: `data/2026-09-26/L30/` (README).

- **Le decisioni dell'utente, conversando e con le cifre in dB SPL:**
  ...
  - «A, e chiudi NC-029».
- **Perché l'interruttore morbido.** Il mute completo dura ~6,6 s (dissolvenza di 6 s più
  0,5 s): tenerlo coi rail dopo la perdita di rete chiederebbe ~0,35 F per rail.
  L'interruttore diventa un ingresso del temporizzatore, e il relè di rete si stacca a mute
  completo. **V2: 12 su 12, peggiore 2,7 µV (~+2 dB).**
- **Il guasto e la perdita di rete.**
  - Sorvegliante a |13,5 V|, tenuta ≥ 1500 µF effettivi, `VRELAY` tenuta ≥ 25 ms.
  - **12 su 12 ≤ 1,77 mV (~58 dB)**: obiettivo 2 mV, tetto 0,87 V (~112 dB).
  - Il controfattuale senza Δ dà 69 mV (~90 dB).
  - Il corto istantaneo di un rail è accettato.
- **Rimisure e spiegazioni.**
  - Regolazione persa col rail + a ~10,6 V; il solo rail − regge fino a −7,5 V.
  - I «4 mV non spiegati» di L29d2 erano il rimbalzo del contatto a rail già in discesa.
- **Il calore**: 15,4 W nominali, 17,8 W nel caso peggiore, ≤ 55 °C nel telaio anche in un
  vano chiuso stretto. **ADR-047**: P5 al numero vero.
- **Il banco** estende il generatore di V2 con `--matrice l30`: sul sorgente, col gemello
  comportamentale di K1/K5. Il deck versionato si rigenera byte-identico. Sorgente toccato solo
  nei commenti (J4, J1).
- **Trappole**:
  - **una `alter` su un dispositivo che non c'è dà «no such device» ed exit 0.** Il primo giro è
    girato senza i contatti del jack, e `corri.sh` lo segnava rc=0. Prima di fidarsi di un giro:
    `grep -l 'no such device' corse/*.log` vuoto;
  - **SKiDL non dà nomi stabili alle net fuse**: a sorgente invariato, due esecuzioni hanno dato
    `BL_IN` / `L_MAINOUT` e `L_ATT_W` / `L_MAINJACK`. Un diff di netlist va letto sulle
    connessioni;
  - **commutare un contatto netto (o un rail in 10 µs) col blocco fuori regolazione** ferma il
    JFET su «Timestep too small». Per il guadagno serve il gemello; i due controfattuali col
    contatto del jack restano non corsi.

### L35 — comandi e LED a pannello nel sorgente, con ADR-045 (2026-09-25)

**Nessuna ADR nuova**: la realizzazione è quella di ADR-045 e ADR-028. **Chiude NC-032**; in
NC-028 chiude il residuo di L36, e la voce resta aperta e bloccante per lo spegnimento (L30).
Report: `reports/2026-09-25-L35-comandi-e-led-a-pannello.md`. Dati: `data/2026-09-25/L35/`
(README).

- **ADR-045 nel sorgente**: la bobina di K6 va su `PERMIT_CMD` (`trim.py`). I due comandi escono
  su **J4** (`MUTE_TIMER`) con l'interruttore di mute. Accanto a J4 c'è il contratto:
  - Δ nominale 20 ms, ≥ 10 ms garantito, contro i 3 ms di rilascio massimo del G6K;
  - al rilascio `PERMIT_CMD` non dopo `MUTE_CMD`;
  - i 13 ms di ADR-027 all'accensione passano su `PERMIT_CMD`;
  - l'OR di F10 e la nota per il failsafe.

  In dB: fino a ~90 dB SPL di picco a 1 m → ~−22 dB, anche per il trim.
- **Il pannello**:
  - **J5** (trim) e **J6** (guadagno) al posto di D4–D9, ref non riusate (#22);
  - **SW3** (`SW_SPST`), ingresso del temporizzatore: chiuso = musica, un filo rotto mette in
    mute;
  - **J7** + **R3**, il LED rosso di mute, letto da `VTRIM` (i due NC di K6): stato di un
    contatto, nessun relè in più. Con ADR-045 sbaglia solo dal lato sicuro;
  - SW1 e SW2 invariati.
- **Il 2e**:
  - «fuori mute» = relè del mute **e** del permissivo eccitati (`command_relays()`);
  - la **finestra Δ** (K6 eccitato, K2–K4 rilasciati) provata per trim, guadagno e LED di mute;
  - i comandi su J4 (`check_commands()`: K6 su `MUTE_CMD` fallisce col suo nome);
  - i LED sui loro header, compresi quelli del trim nei quattro stati dei bistabili, che prima
    non erano provati (`reach(..., latched=)`).

  14 falsi e `main` falliscono. Regressione: L16, L29e e L36 come allora. Due falsi non si
  applicano più: `bobina_K6_altra_net` di L16 oggi è il progetto, e `led_scambiati` di L36
  sabotava D8/D9.
- **Invariati**: ERC 47 avvisi, deck V2 byte-identico, `run_tests.sh` 10/0.
- **Budget**: le bobine restano **168,8 mA** a 5 V. I LED sono ~4 mA fuori mute e ~6 mA in mute:
  il commento di prima li contava solo in mute. Caso peggiore **~175 mA**.
- **Trappole**:
  - nel worktree `git show --output=` scrive un file vuoto e la netlist finisce sullo schermo:
    per le netlist storiche serve uno script Python che chiami `git show` e scriva il file;
  - il falso di L16 `bobina_K6_altra_net` era, parola per parola, la decisione di ADR-045: un
    falso vecchio che diventa progetto va tolto dalla regressione e sostituito, non «corretto».

### Dopo L36 — il residuo non si accetta: K6 sfasato dopo i relè del jack (2026-09-25)

**ADR-045.** Detto in dB (preferenza dell'utente: i bump in dB SPL contro una stanza silenziosa,
non in mV), il residuo di L36 vale fino a ~90 dB SPL di picco a 1 m, contro ~33 dB di V2. È la
manopola girata fuori mute e poi il mute inserito, con K6 e K2–K4 che rilasciano insieme.
- L'utente: «non possiamo accettare il comportamento», poi «sono d'accordo con lo sfasamento e
  metti una nota per il failsafe».
- **La decisione**: K6 su `PERMIT_CMD`, rilasciato Δ (~20 ms) dopo `MUTE_CMD` all'inserimento, ed
  eccitato non dopo `MUTE_CMD` al rilascio. L'ordine poggia sul rilascio massimo di 3 ms del
  datasheet. Chiude anche il caso gemello del trim. Lo realizza **L35**, come primo punto, 2e
  compreso.
- **La nota per il failsafe**: alla caduta di `VRELAY` lo sfasamento sparisce. Il failsafe di L30
  deve rilasciare `MUTE_CMD` per primo e tenere K6, K1/K5 e K11/K12 almeno Δ dopo (ADR-043).
- NC-028 aggiornata. Il residuo resta nel sorgente finché L35 non lo realizza.

### L36 — il guadagno interbloccato dal mute (2026-09-25)

**Nessuna ADR nuova**: è la strada B di ADR-030 nella forma di ADR-041. NC-028 resta aperta e
bloccante per lo spegnimento (L30), con un residuo registrato. Report:
`reports/2026-09-25-L36-guadagno-interbloccato.md`. Dati: `data/2026-09-25/L36/` (README).

- **Il vincolo che ha deciso la forma**: il datasheet del G6K (p. 3) dà solo «operate/release 3 ms
  max». Non dà induttanza, rilascio minimo né trasferimento, quindi la corsa di ADR-030 non si
  chiude con i tempi.
- **Il sorgente** (`gain_interlock.py`):
  - K1+K11 e K5+K12 fra `G3_HI`/`G10_HI` e `RLY_RET`, pin 1 + (p. 6, riletta a 300 dpi);
  - comando da `VTRIM` via SW2 e Schottky;
  - **ponte** da `VRELAY` via SW2 e il NO dell'ausiliario;
  - tenuta da `VHOLD` (NO del polo 1 di K6) via diodo e il NO dell'ausiliario;
  - LED sul polo 2 degli ausiliari;
  - ref nuove esplicite: K11, K12, SW2, R2, D7–D15. ERC da 42 a 47 avvisi, tutti spiegati.
- **Il 2e**:
  - ruolo HOLD e diodi orientati;
  - SW2 provato posizione per posizione contro la tabella di ADR-026;
  - prova fuori mute, in mute e **nel trasferimento di K2–K4/K6** (la corsa);
  - LED, uno per stato.
  
  11 falsi e `main` falliscono ciascuno per la ragione voluta. I falsi di L16 e L29e danno gli
  esiti di allora.
- **La corsa simulata**, 1440 corse rc 0, nessuna col transient op. Col ponte **0 cadute su 480**;
  senza, **373**, e dove dipende proprio da L, soglia e buco.
- **Deck V2** rigenerato **byte-identico**: il segnale non è toccato.
- **Budget**: 8 bobine a +10 dB in entrambe le condizioni, **168,8 mA** a 5 V (era 126,6).
  Vincolo: VRELAY − V_F(Schottky, 42 mA) ≥ 80 % di 5 V, a −5 % e a caldo.
- **Il residuo, per l'utente**: la manopola girata **fuori** mute e poi il mute inserito. Il
  guadagno cambia un rilascio di K1 dopo l'apertura del NO di K6. La parte elettrica simulata va
  da 2,6 µs a 1,3 ms; la meccanica non è accreditata. Tre strade (sezione 7 del report):
  accettarlo come il trim e misurarlo al prototipo; la regola d'uso; un permissivo in più, che
  riapre ADR-030.
- **La proposta per il trim** (non applicata): la stessa strada col ponte, a parità di relè. Costa
  fino a +84,4 mA fuori mute e rifà L16. Raccomandazione: quasi un pareggio, si tiene L16 salvo
  preferenza dell'utente.
- **Trappole**:
  - SKiDL 2.3 non esporta un marcatore `NC`: i pin liberi restano scollegati e l'ERC li segnala;
  - `falsi.py` di L16 scrive nella propria cartella dati: si fa girare su una copia della radice,
    con `library/`, altrimenti SKiDL non trova la libreria `preamp`;
  - un pin libero non si scambia con `scambia_pin` (trova un nodo solo): per quello serve
    `rinumera`.

### L29e — la geometria iii nel sorgente (2026-09-25)

**Nessuna ADR nuova. La parte del mute di NC-028 è confermata sul sorgente**; NC-028 resta aperta
e bloccante per lo spegnimento (L30). Report: `reports/2026-09-25-L29e-geometria-iii-sorgente.md`.
Dati: `data/2026-09-25/L29e/` (README).

- **La decisione dell'utente**: il bleed dal lato del condensatore, «**Tenerlo**». 470 k sulle
  fisse (`R167`, `R168`, `R367`, `R368`), 220 k sulla principale (`R264`, `R464`), con ref
  esplicite: nessuna parte si rinumera.
- **La piedinatura**, riletta a 600 dpi: il G6K-2F-Y è a **pagina 6** del PDF, non 7 (quella è il
  G6KU). Perno 3/6, riposo 2/7, aperto 4/5: coincide con la mappa del sorgente.
- **Il cablaggio**: ogni polo di K2–K4 è un deviatore. COM al lato del condensatore (nodi nuovi
  `*_FIXCk`, `*_MAINC`), NC a massa, NO al jack, unito al connettore. Il bleed del jack resta sul
  jack. ERC da 48 a 42 avvisi: spariscono i 6 NO scollegati.
- **P7, riletto**: in mute va a massa il lato C invece del jack, e lo stadio vede lo stesso
  carico (47 Ω + 4,7 µF verso massa). `tb_mute_corto.cir` (0,01 Ω) resta il caso peggiore; la
  nota è nel sorgente e nel deck.
- **Il 2e** (`check_relay_safe_state.py`) asserisce la geometria per intento: condensatore sul
  COM, connettore sul NO, bleed su entrambi. Fatto fallire sulla netlist di `main` e su 4 falsi.
- **Il deck versionato** si genera dalla netlist (`--matrice sorgente`, il default nuovo del
  generatore di L29c): i bleed lato C vengono dalla netlist, e il generatore rifiuta se non è la
  iii. `l29c` e `l29d2` si rigenerano byte-identici; il deck nuovo è la variante di progetto di
  L29d2, corsa per corsa (135 su 135).
- **Le celle peggiori sul sorgente**: 17 corse rc=0, nessun transient op, contatto a 1e-13 F.
  **18 verdetti su 18 uguali al banco, scarto 0, 0 fuori.** Cambio a relè chiuso 0,17 µV,
  dispersione 0,49 / 69,7 µV, accensione 4,4 µV, musica 7,16 / 5,45 dB.
- **Il diagramma a blocchi** mostra il deviatore in serie.
- **Trappole**:
  - nel worktree anche i comandi con `$CLAUDE_JOB_DIR` e le pipe attorno a `zsh` vengono
    rifiutati: percorsi letterali, un comando per chiamata;
  - `L29d2/script/identici.sh` non vale più per il deck versionato, che ora è `sorgente`: si
    confronta `--matrice l29c` scritto altrove;
  - le celle sulla iii sono durate 14 min al massimo, non 36.

### L29d2 — la matrice del contatto in serie sulla geometria iii (2026-09-25)

**ADR nuova: ADR-044. La parte del mute di NC-028 è chiusa** (decisione dell'utente). **NC-028
resta aperta e bloccante per lo spegnimento** (L30). Report:
`reports/2026-09-25-L29d2-matrice-contatto-serie.md`. Dati: `data/2026-09-25/L29d2/` (README).

- **Le decisioni dell'utente**, prima di correre, una domanda alla volta:
  - geometria «iii»;
  - stato sicuro «Il bleed basta»;
  - valori «C dal datasheet + cavo realistico».

  A riposo il jack va a massa attraverso il **suo** bleed (`RBLx`). La domanda aveva scritto
  «bleed lato condensatore», ed era imprecisa: il report lo dice.
- **La capacità del contatto aperto** si ricava dalla curva d'isolamento del G6K, che è capacitiva
  pura (20,2 dB/decade): **0,075–0,080 pF**, e nel banco **0,1 pF** (`script/c_contatto.py`).
- **Il banco**: il generatore di L29c esteso (`--matrice l29d2`), 4 varianti in un deck, scelte
  col regex:
  - 0 pF e 100 k;
  - cavo 100 pF;
  - 10 k;
  - 5 pF, il ponte con L29d.

  Gli 8 deck versionati di L29c e L29d si rigenerano byte-identici (`script/identici.sh`).
  Controfattuale N = L29c (111,5 → 111,5 µV).
- **Risultati**: 111 corse di matrice e 43 di varianti, tutte rc=0 e partite dal punto giusto.
  **0 fuori su 253**; nelle varianti di progetto 0 fuori su 47.
  - Cambio di guadagno a relè chiuso da 111,5 a **0,17 µV**, trim da 101,7 a **0,10 µV**.
  - Dispersione, sul cambio, da 267 a **≤ 0,60 µV**.
  - Accensione da 11 280 a **4,4 µV**.
  - Con musica S 7,16 dB e B2 ≤ 8,7 µV, invariati.
  - Il cavo e i 10 kΩ abbassano tutto.
  - Il ponte `k5` ridà i 205 µV di L29d.
  - La cella più alta, **69,7 µV**, è la dissolvenza delle LDR con la dispersione: identica a L29c,
    e non viene dal contatto.
- **Lo spegnimento** (diagnostica per L30): ancora fuori, da 5 µV a 2,3 V.
  - 7 corse su 24 si fermano su «Timestep too small» nel ms del trasferimento: `gear` non aiuta.
  - I 3,96 mV col relè immediato non sono spiegati.
- **Per il circuito**:
  - la iii è **un deviatore per uscita** sul G6K esistente: comune al lato del condensatore, NC a
    massa, NO al jack. È un ragionamento, e lo porta **L29e**;
  - **vincolo di layout (G2)**: fra il lato del condensatore e il jack stare sotto **~2,4 pF**,
    contatto e piste insieme (~41 µV/pF all'accensione).
- **Decisioni dell'utente dopo i risultati**: «chiudi la parte mute di NC-028, e L29e prima di L36,
  concordo con la tua proposta».
- **Trappole**:
  - nel worktree, anche un `sed` con più comandi `-e` e un `a\` viene rifiutato: si usa Edit;
  - le corse di dispersione e di mute a 1 kHz sulla iii durano fino a 36 min l'una, 6 volte
    L29c: si pianificano per prime.
...
**NC-028 resta aperta e bloccante. ADR nuova: ADR-043.** Report:
`reports/2026-09-23-L29c-v2-caso-peggiore.md`. Dati: `data/2026-09-23/L29c/` (README).

- **Il banco**: `spice/preamp/tb/tb_v2_casopeggiore.cir`, **generato** da
  `data/2026-09-23/L29c/deck/genera_tb_v2_casopeggiore.py` (246 corse; più i deck `caldo/` e
  `curve/`). Contatti K1/K5 e trim che commutano nel tempo, VOS e gruppo B, rail in rampa, tutto
  fuori dal blocco CANALE.
  - La cella di L40 rifatta è **identica**.
  - Il banco in posizione neutra la ridà **entro 0,09 %**.
...
- **Fuori dai 100 µV di A, senza musica**, per una causa sola: il contatto di mute in derivazione,
  0,1 Ω dietro 47 Ω, attenua ~1/471 un salto in continua dell'uscita:
  ...
  - lo spegnimento: da 0,23 mV a 17,5 V (sotto ~10 V l'uscita scatta verso il rail rimasto).
- **Decisioni dell'utente**:
  - (b) lo spegnimento va all'alimentatore, «ma dobbiamo anche prevedere un failsafe mechanism in
    cui l'alimentatore si rompa» → **ADR-043**;
  - poi, sulla proposta: un **contatto in serie** al jack, da misurare in **L29d** prima di L36,
    in due varianti (serie più derivazione; serie sola). Le LDR restano: S le vuole.
- **Trappole**:
  - **limitations #33**: il «transient op» di ngspice finisce «successfully» con lo stadio
    agganciato al rail; `corri.sh` lo rifiuta, e riprova senza `.nodeset` e con un `.nodeset`
    esteso;
  - l'interruttore netto dentro l'anello attivo si ferma a un rimbalzo (il cambio a caldo va
    col contatto morbido, stesso A entro 0,4 %);
...
**Chiude NC-017; apre NC-034 (bloccante) e NC-035.** Report:
`reports/2026-09-22-L39-modelli-costruttore.md`. Dati: `data/2026-09-22/L39/` (`prima/`
sui segnaposto, `dopo/` sui modelli del costruttore, stessi deck, stessa sessione).
Nessuna ADR, nessun valore del circuito cambiato.

- **La sostituzione.** `gain_block.py` istanzia `LSK489A`, `MMBT5551`, `MMBT5401`,
  `Qmje15032`, `Qmje15033`, `D1N914` (più `LS350`); valori e footprint MMBT in SOT-23
  (ADR-017). Nel blocco generato cambiano **solo i nomi dei modelli, 12 righe**. I 25
  deck includono i sei file di `models/`; intestazioni riscritte, 2h verde.
- **Il derivato di L20 è ritirato** (il blocco nomina già `LSK489A`); il **2i** è
  `scripts/check_no_placeholders.py`: 49 violazioni sul `main` di prima, 8 casi su 8.
  `placeholder_devices.lib` resta per i deck datati, annotato.
- **Reggono**: E2 (+3,039 / +9,959 dB), E3 (≥ 110,7 kΩ), E4 (60,04 / 53,12 Ω), E5
  (≤ 5,050 µV, pavimento: 1/f solo sull'LSK489A), V3 (recupero 1,07 µs), P7 (MJE
  0,362 W), classe A sui percorsi ascoltabili, V2 dei relè, V2 del mute a 1 kHz
  (S 7,16 / 5,32 dB, A ≤ 3,75 µV).
- **Il fondo di distorsione della catena** (C_pav, principale, 1 kHz) scende da 0,96 a
  **0,27 mV**. L'offset del blocco B da −16,58 a **−15,45 mV**.
- **V1 cade** di 5,5–7° su ogni istanza a guadagno unitario → **NC-034**. C_f non
  aiuta a 0 dB; il Miller da 470 a 820 pF riporta il blocco B a 60,70° con il
  crossover da 889 a 520 kHz (esplorazione, non applicata).
- **Decisione dell'utente**, a metà lotto: «non sono convinto che ci serva quasi un
  megaherz di banda passante a 0 dB, preferisco rispettare i margini di fase e ridurre
  la banda passante o alzare il guadagno fino a 1,5 dB». E NC-017 si chiude su T7, con
  V1 in una voce nuova. La correzione è di **L40**, prima di L29c.
- **Corrente di riposo** 14,56 → 20,29 / 20,40 mA: la Vbe dei MJE veri è 0,57 / 0,54 V.
  Lo sweep del moltiplicatore da 1,5 a 1,87 kΩ dà 17,3–23,2 mA → **NC-035**.
- **Trappole**: il blocco A coi modelli veri non converge per gmin stepping e ci arriva
  per source stepping (50 su 50); il manifesto di `tb_v2_mute_ldr` ha righe che non
  sono corse (`rele_*` rilegge `ev`, `pav_*` legge `evp`/`invp`), quindi una cella si
  estrae dalla colonna `file`, non dalla colonna `cella`.

Suite **10 passed / 0 failed**. **14 voci aperte, 3 bloccanti.** Prossimo: **L40**.

### L29b2 — il mute LDR nel sorgente, e il taglio giudicato sul salto di livello (2026-09-22)

**NC-028 resta aperta e bloccante**: manca il caso peggiore (L29c). Report:
`reports/2026-09-22-L29b2-mute-ldr-sorgente.md`. Dati: `data/2026-09-22/L29b2/`. ADR
nuove: **ADR-039**, **ADR-040** e **ADR-041** (il guadagno si interblocca col mute come
il trim, con LED: la forma di L36 è decisa dall'utente).

- **Il pavimento di mV «con le LDR attive» era `wrdata`.** Scrive il tempo con 9 cifre
  significative, cioè 100 ns sopra 10 s. Fra due corse con griglie diverse
  l'arrotondamento dava 4,6–7,2 mV di C2. Con `set numdgt=15`: ≤ 31 µV lungo tutta la
  sequenza (**limitations #30**). Probabilmente è anche il pavimento «non attribuito»
  di L29a. TMAX falsificato; né `gear` né varianti del modello sono serviti.
- **v3 decisa**: al rilascio la cella in serie si accende alla velocità del LED, e il
  livello salta da −62 a −26 dB in 150 ms (**30 dB in 100 ms**; 29,7 dB a 20 Hz sulla
  matrice).
- **C ≤ 1 mV non si può soddisfare**, misurato col simulatore veloce tarato su ngspice
  (`transizione/`, celle entro 0,002 decadi):
  - a 20 Hz nemmeno un coseno ideale sotto ~14 s;
  - la distorsione della catena vale da sola 0,65–0,96 mV sulla principale;
  - le celle non fanno un coseno in 6 s (0,4 decadi/s sopra 80 kΩ).
- **ADR-040, dall'utente**: «il Technics che andiamo a sostituire ha uno switch di
  pseudo mute di 20 dB e non mi ha mai dato fastidio». Il taglio con musica si giudica
  su **S, il salto di livello, ≤ 20 dB in 100 ms** (sopra −70 dB). C2 diventa
  diagnostica. S è in `v2_metodo.py`: 30 controlli, 14 sabotaggi su 14.
- **Profilo v4** (proposta di L29b2, adottata): la serie scende da 4,5 µA al ginocchio
  del buio, 0,19 µA, fino a d = 0,75. Nel sorgente (ADR-039) come contratto accanto a
  J3.
- **Sorgente**: U101/U102 e U301/U302, LED in serie fra i canali su **J3 `LDR_CMD`**.
  2e, 2f e 2g estesi e fatti fallire (F1–F3, `main` senza LDR, porta morta, dispositivo
  morto).
- **E3** ≥ 111,6 kΩ, **E5** ≤ 4,957 µV (`tb_e3_e5_ldr.cir`), **P7** regge per
  ragionamento. Requisito per il comando: rumore all'anodo ≤ 0,69 mV/√Hz.
- **Matrice v4** del deck versionato `tb_v2_mute_ldr.cir` (tre uscite, 10/100 kΩ,
  20 Hz / 1 kHz; 20 kHz solo 100 kΩ, scelta dell'utente):
  - **S ≤ 7,2 dB** a 20 Hz e 1 kHz (inserzione 7,2, rilascio ≤ 5,3, inversione ≤ 3,0);
  - a 20 kHz S ≤ 6,6 dB (inserzione 6,6, rilascio 6,1, inversione ≤ 2,3); il relè S = 0 ovunque;
  - A ≤ 3,9 µV, B2 ≤ 10 µV.
...
- **Trappole nuove**:
  ...
  - le corse a 20 kHz con TMAX 0,5 µs valgono **circa 4 ore** l'una con 7 in parallelo su 10 core, misurato; il ritmo dei primi minuti inganna, e un'analisi Python in parallelo lo dimezza;
...
**NC-028 sale a bloccante.** Report: `reports/2026-09-15-L38-risposte-requisiti.md`.
Calcolo: `data/2026-09-15/L38/`. Nessun deck eseguito, nessun valore del circuito.
ADR nuove: **ADR-032**, **ADR-033**, **ADR-034**.

- **La soglia letta contro i numeri, prima di scriverla** (la lezione di L37).
  - 100 µV valgono **33,45** dB SPL calcolati, non «circa 34».
  - Un gradino netto passa il filtro quasi intero; l'ordine del passa-alto pesa
    solo sulle rampe, circa ×2 per ordine.
  - Letta alla lettera, la soglia avrebbe bocciato per il residuo a mute inserito
    (4 mV p-p) ogni mute a contatto singolo. E i banchi usano contatti a 0,01 Ω,
    contro i 100 mΩ massimi del datasheet G6K.
- **Chiesto all'utente, che ha risposto.**
  - La soglia vale per **A, B e C**: gradino, residuo in mute, taglio della
    musica.
  - Spegnimento: «si lo spegnimento rientra».
  - Ordine del filtro: «indifferente», quindi 2° ordine.
  - Finestra di C: 10 ms.
- **ADR-032 e V2.** Soglia e metodo scritti in V2. Il criterio 3 di ADR-030 si
  legge nel caso peggiore, e una regola d'uso non rende conforme V2.
- **NC-028 è decidibile, e il verdetto è negativo.** Il CSV versionato di L11
  porta 5,37 V al jack. La voce sale a **bloccante**, come NC-002 in L26, e resta
  aperta per L29.
- **NC-019: il ponte di rame, alla lettera, era un corto.**
  - Il tab del MJE15032 è su `VP`, e `TO-220-3_Vertical` non ha la piazzola del
    tab.
  - ADR-034 registra la risposta dell'utente: «Rame, forma da dimensionare».
  - `gain_block.py` ha ricevuto solo un commento: AST identico a `main`, e il
    controllo è stato fatto fallire.
  - La consegna a `pcb-automation-engineer` è solo quel commento. NC-019 resta
    aperta.
- **ADR-033**: LED dai relè spia, e il guasto di un solo relè è accettato.
- **NC-009**: il punto 3 si sposta sul manuale d'uso, che non esiste. Resta
  aperta.
- **Il Singxer** è chiuso in «Aperti»; il condensatore del phono resta rinviato.
- **L'indice delle ADR** segna ADR-017, ADR-027 e ADR-030 come precisate; i loro
  file sono intatti.

Suite **10 passed / 0 failed**. **13 voci aperte, 3 bloccanti.** Prossimo:
**L29**.
...
| L28 | **SS dell'LSK489 definito per l'LSK489.** Un documento del costruttore che dica cosa sono i pin 3 e 7 di *questa* parte — o una ADR che accetti l'istruzione dell'LSK389 in forza della compatibilità dichiarata. Prima di G2 | XS/S | NC-027 | da fare — **la fonte c'è** (2026-09-22, ricerca dell'utente verificata in L29b2): Bob Cordell, *LSK489 Application Note*, Rev A2, pag. 6, «The Common Substrate», dal sito del costruttore (`https://www.linearsystems.com/_files/ugd/4be30b_49c5a96bc52f4868a7bf2a5c17150351.pdf`, SHA-256 `84f21b11763404e7fd4c9d8d461a86a833b0aea9c8459fdd9babe777149f1d8f`): SS è il substrato comune, i diodi hanno l'anodo sui gate e il catodo sul substrato, «it harmlessly floats», e nel SOIC «may» collegarsi a una tensione fissa. **Quindi pin 3 e 7 flottanti come oggi; se mai collegati, sopra i gate, MAI al negativo** (un testo di un altro modello diceva il contrario, e i pin 1 e 8: sbagliato). Il datasheet sul sito è identico a Rev A40 in `vendor/`. Resta: congelare la nota con `freeze_vendor.sh`, un'ADR, chiudere NC-027; nessuna modifica al circuito |
...
| L41a | **L'alimentatore, prima parte** (NC-036, ADR-046): le scelte dell'utente conversando, poi il sorgente della potenza, di `VRELAY`, del relè di rete e del sorvegliante; simulato da solo; controllo del cablaggio fra le schede; P2 avviato | M | NC-036 | **fatto** — **ADR-048**: due trasformatori standard, rail lineari, `VRELAY` 12 V da un avvolgimento proprio, standby con interruttore posteriore, sorvegliante in hardware (rail, rete, `VRELAY`), temporizzatore ibrido, P5 ≤ 20 W. `circuits/preamp/psu.py`, `check_psu_harness.py` (2j), `SAFETY.md`. Perdita di rete: jack staccato in 14 ms coi rail a 15 V. Apre NC-037. Report `reports/2026-09-26-L41a-alimentatore-potenza-sorvegliante.md` |
| L41b | **Il temporizzatore** (ADR-048 punto 6): il micro (sequenza del mute, profilo v4 delle LDR col convertitore esponenziale, accensione dallo standby coi 13 ms, spegnimento morbido, OR con SW3, debounce, rilascio dopo un buco di rete, ritenuta dopo un guasto), il ritardo Δ in hardware su `PERMIT_CMD`, il taglio di `VRELAY` alla scheda audio in standby (NC-037). Al posto del segnaposto J509 | M | NC-036, NC-037 | **diviso** in L41b1 e L41b2 (decisione dell'utente, ADR-049) |
| L41b1 | **Il temporizzatore, l'hardware**: le scelte dell'utente (micro, dove vive il firmware), il micro e il DAC al posto di J509, Δ e Δ₂ in hardware, l'interruttore di `VRELAY` per lo standby, il pilota esponenziale delle LDR; la specifica scritta; Δ col micro fermo, il profilo in temperatura, il rumore; il 2j e il 2e estesi | M | NC-036, NC-037 | **fatto** — **ADR-049**: ATtiny3216 + MCP4822, `firmware/preamp_timer/spec/timer_spec.md`. Δ ≥ 16,9 ms col micro in reset / a zero / in perdita di rete / col guasto di U503, controfattuale 0; standby 0 V a J1, 92,5 mW dal secondario; LDR calibrate ±0,92 dB 15–60 °C. 2j + 2e `--timer`: 15 falsi e `main` falliscono. NC-037 aperta col criterio di T2; apre NC-038. Report `reports/2026-09-26-L41b1-temporizzatore-hardware.md` |
| L41b2 | **Il temporizzatore, il firmware**: `firmware/preamp_timer/src/timer_core.c` secondo la specifica, i test sull'host (`clang`) uno per sequenza e ciascuno fatto fallire, le forme d'onda del core come micro comportamentale del banco di L41b1, la sequenza sul circuito (accensione, rilascio, inserzione, spegnimento, buco di rete, guasto); la verifica della SPI su PA2 | M | NC-036 | da fare — **prossimo**. Mandato in `NEXT-SESSION.md` |
| L41c | **Il banco di L30 col circuito vero** (NC-036): l'alimentatore di `psu.py` al posto delle PWL `@vpp`/`@vmm`/`@vpwl` e degli istanti dei contatti, col metodo di V2: spegnimento morbido sotto V2; perdita di rete, regolatore aperto, rail solo e `VRELAY` persa sotto l'obiettivo di 2 mV; il controfattuale senza Δ che fallisce. Con l'utente: il corto dell'uscita di U503 (~90 dB SPL, sotto il tetto) | M | **NC-036** | da fare |
...
| L29d2 | **La matrice del contatto in serie sulle geometrie scelte dall'utente** (NC-028). Prima le decisioni sulla sonda di L29d: quale geometria (iii, iB con un trasferimento più lungo); lo stato sicuro di ADR-012 con la derivazione lato condensatore; i valori (bleed lato condensatore, capacità del contatto aperto dal datasheet, cavo, trasferimento). Poi la matrice di L29c (guadagno, trim, dispersione `dp…max`, accensione, musica S/B2 a 1 kHz e 20 Hz, B col contatto aperto, **10 kΩ**). La scelta diventa una ADR (tocca ADR-012 e il failsafe di ADR-043) | M | NC-028 | **fatto** — decisioni dell'utente: «iii», «Il bleed basta», «C dal datasheet + cavo realistico» (0,1 pF). 0 fuori su 253, 0 su 47 nelle varianti; cambio a relè chiuso 0,17 µV, accensione 4,4 µV. **ADR-044**; la parte del mute di NC-028 chiusa dall'utente; lo spegnimento resta a L30. Report `reports/2026-09-25-L29d2-matrice-contatto-serie.md` |
| L29e | **La geometria iii nel sorgente** (ADR-044; decisione dell'utente del 2026-09-25: «L29e prima di L36»). In `circuits/preamp/`: il relè del mute al jack usato come deviatore per uscita (comune al lato del condensatore, NC a massa, NO al jack), piedinatura del G6K-2F-Y letta dal datasheet (trappola NC-014); il bleed lato condensatore deciso con l'utente; ERC, 2e e il deck versionato riletti; le celle peggiori di L29d2 (cambio a relè chiuso, dispersione, accensione, musica) rifatte sul sorgente. Se non si riproducono, la parte del mute di NC-028 si riapre | S–M | NC-028 (conferma della parte del mute) | **fatto** — il bleed lato C tenuto dall'utente («Tenerlo»); deviatore K2–K4 nel sorgente, piedinatura dal datasheet (p. 6); 2e esteso alla geometria e fatto fallire; deck versionato generato dalla netlist; celle peggiori sul sorgente 18 su 18 uguali al banco, 0 fuori: la parte del mute di NC-028 **confermata**. Report `reports/2026-09-25-L29e-geometria-iii-sorgente.md` |
| L30 | **Il calore del telaio con otto blocchi.** A riposo la scheda audio dissipa 6,45 W (0,806 W per blocco, L17) contro i 3-4 W che P5 prevede per l'apparecchio intero. Stima termica del telaio con l'alimentatore; poi o P5 aggiornato al numero vero, o ventilazione e montaggio progettati con una ADR, o ADR-021 riaperta se i 60 °C non reggono. Va col lotto dell'alimentatore. **Da ADR-043 (L29c)**: lo spegnimento è un requisito dell'alimentatore (rail audio sopra ~10 V finché il mute non è completo: supervisore alla perdita di rete e tenuta dei rail, verificati col metodo di V2) e serve un **failsafe per il guasto dell'alimentatore** che non dipenda dall'alimentatore sano; forma da progettare con l'utente, dopo L29d; soglia in caso di guasto da chiedere. **Da ADR-045 (dopo L36)**: alla caduta di `VRELAY` lo sfasamento di K6 sparisce e K1/K5 tornano a 0 dB insieme allo stacco dei jack; il failsafe rilascia `MUTE_CMD` per primo e tiene K6, K1/K5 e K11/K12 almeno Δ dopo, o prova un'altra via. **Da L35**: il contratto del temporizzatore scritto accanto a J4 in `preamp_audio.py` (Δ, l'OR di F10 con SW3, i 13 ms su `PERMIT_CMD`) | S | NC-029, NC-028 (spegnimento) | **fatto** — l'utente: soglia di non-danno nel guasto («come tetto, non come target»), interruttore morbido, forma del failsafe. **ADR-046** e **P9**: spegnimento morbido, V2 12 su 12 ≤ 2,7 µV; perdita di rete come guasto, 12 su 12 ≤ 1,77 mV (~58 dB) contro l'obiettivo di 2 mV. **ADR-047**: P5 a 15–18 W, 60 °C regge. Banco `data/2026-09-26/L30/`, deck V2 byte-identico. **Chiude NC-028 (strada A) e NC-029, apre NC-036** → L41. Report `reports/2026-09-26-L30-spegnimento-failsafe-calore.md` |
...
| L35 | **Comandi e LED a pannello nel sorgente** (ADR-028). Header di cablaggio al posto dei LED del trim sulla scheda (`trim.py`); comando del guadagno sul rotativo a 3 posizioni: **il cablaggio di SW2 esiste da L36** (4 poli, comando e ponte per bobina, ADR-026 provata dal 2e), resta portarlo a pannello coi LED del guadagno D7–D9; interruttore di mute combinato col temporizzatore d'accensione; LED rosso di mute, letto da un contatto che dica lo stato; `check_relay_safe_state.py` esteso. **Dopo L29 e L36**: il comando del guadagno si cabla una volta sola | S/M | **NC-032**, parte di NC-028 (ADR-045) | **fatto** — K6 su `PERMIT_CMD` con J4 e il contratto del temporizzatore (Δ ≥ 10 ms, nominale 20 ms); J5/J6 al posto dei LED sulla scheda, SW3 ingresso del temporizzatore, J7 + R3 LED di mute da `VTRIM`. 2e esteso (comandi, finestra Δ, LED a pannello), 14 falsi e `main` falliscono; deck V2 identico, ERC 47. **Chiude NC-032** e il residuo di L36 in NC-028. Report `reports/2026-09-25-L35-comandi-e-led-a-pannello.md` |
| L36 | **Il guadagno interbloccato dal mute** (ADR-030, strada B). **Non più «solo se» (ADR-038)**: guadagno e trim si cambiano solo col jack a massa, a musica spenta; **La forma è decisa (ADR-041, dall'utente il 2026-09-22): interblocco da premere, come il trim; niente mute automatico; LED anche per il guadagno.** Due relè ausiliari con autoritenuta su K1/K5; la corsa al rilascio del mute (scambio di K6 contro rilascio delle bobine) simulata e chiusa; il 2e esteso al guadagno; tre LED dello stato vero dai poli liberi degli ausiliari; budget delle bobine (~169 mA fuori mute a +10 dB, a 5 V) consegnato all'alimentatore. Se la corsa si chiude bene, rivalutare la stessa strada per il trim | M | parte di **NC-028** | **fatto** — `gain_interlock.py`: ausiliari K11/K12 in autoritenuta da `VHOLD` (NO di K6), SW2 con un polo ponte per bobina, LED D7–D9 dagli ausiliari. Corsa chiusa **per struttura** (il datasheet non dà la dinamica della bobina): provata dal 2e nel trasferimento, simulata 0 cadute su 480 contro 373 senza ponte. 11 falsi + `main` falliscono; deck V2 identico; budget 168,8 mA a 5 V. Residuo (manopola girata fuori mute) all'utente, in NC-028. Report `reports/2026-09-25-L36-guadagno-interbloccato.md` |
...
| L38 | **Le risposte del 2026-09-15 diventano requisiti** (voce di diario «Dopo L20»). ADR per la soglia di V2 (≤ 100 µV di picco 20 Hz–20 kHz su tutte le uscite, accensione compresa, cono libero sotto 20 Hz) e V2 in `REQUIREMENTS.md`; criterio 3 di ADR-030 al caso peggiore; NC-028 e L29 aggiornati; NC-009 punto 3 verso il manuale d'uso; NC-019 col ponte di rame, scritto in `gain_block.py` e nella consegna a `pcb-automation-engineer`; LED gemelli accettati (nota in ADR nuova, ADR-027 non si riscrive); Singxer chiuso in «Aperti». **Prima di L29** | XS/S | aggiorna NC-009 e NC-019; NC-028 sale a **bloccante** | **fatto** — ADR-032 (soglia e metodo di V2: A gradino, B residuo in mute, C taglio della musica; spegnimento compreso), ADR-033, ADR-034; il ponte galvanico era un corto (tab su VP); `gain_block.py` solo commento, AST identico e controllo fatto fallire; NC-009 e NC-019 restano aperte |
...
**Aggiornato il 2026-09-25.** Il prossimo è **L30** (riga L30 della tabella dei lotti, prompt in `NEXT-SESSION.md`), col lotto dell'alimentatore; **L28** resta da fare prima di G2. Il testo sotto è storico: il 2026-09-22 il prossimo era **L39, la Fase 4** (riga L39 della tabella dei lotti, prompt in `NEXT-SESSION.md`); poi **L29c**, il caso peggiore di V2 col mute reale (bozza del mandato: `data/2026-09-22/L29b2/bozza_prompt_L29c.md`). Quello che segue è il testo scritto per L29, tenuto come storico.

**L29 — Il gradino, il residuo e il taglio al jack, misurati col metodo di V2.**
Riguarda NC-028, **bloccante** da L38. Lotto **M/L**: se non entra in una
sessione, si divide e lo si scrive qui. Il mandato completo è in
`NEXT-SESSION.md`.

**Perché adesso.**
- **È sul cammino critico**: L36 e L35 aspettano il suo confronto, e NC-028 è
  bloccante.
- **La soglia esiste** (L38, ADR-032). Gli altri lotti aspettano altro: L28 una
  sessione interattiva con l'utente sui pin SS, L30 l'alimentatore.

**Dove parte.**
- **V2 in `REQUIREMENTS.md`**: la soglia, le grandezze A, B e C, e il metodo, da
  seguire alla lettera. Il perché sta in ADR-032.
- **NC-028**: le varianti di mute simulate in scratch dopo L11, da rifare con un
  deck versionato.
- **I nomi dei nodi.** `tb_mute_corto.cir` chiama i jack `JM`, `J1` e `J2`, con
  contatti a **RON 0,01 Ω**. `tb_e4_uscite.cir` li chiama `MAINJACK`,
  `FIXJACK1` e `FIXJACK2`, e `preamp_audio.py` ha `MAINJACK`: il nome si legge
  nel deck, prima di scrivere il marcatore. V2 vuole contatti a 100 mΩ.
- **Nessuna cifra di oggi sta sotto soglia.** Il cambio a caldo, 6–114 mV, è
  calcolato. Il taglio della musica (C) non è mai stato misurato.
...
- **I conteggi**: 13 voci aperte, 3 bloccanti. L38 ha portato NC-028 a bloccante;
  L37 aveva chiuso NC-033.
- **V2 ha soglia e metodo** (L38, ADR-032). Altre decisioni di L38: i LED del
  trim restano sui relè spia (ADR-033); il moltiplicatore di Vbe si accoppia col
  rame, in una forma che dimensiona `pcb-automation-engineer` (ADR-034, NC-019).
  Il manuale d'uso, dove va il punto 3 di NC-009, non esiste e nessun lotto lo
  produce.
...
- **L29** (NC-028) è il prossimo, qui sopra. Dal suo confronto fra cambio di
  guadagno a caldo e cambio sotto mute dipende **L36**.
...
- **L'alimentatore**, quando arriva, parte da **ADR-020** e da NC-029. Deve
  ...
  - alimentare le bobine col budget di L36: **168,8 mA** a 5 V a +10 dB, fuori
    mute e in mute (72,8 mA a 12 V, 36,8 mA a 24 V), più i LED (L35): ~4 mA
    fuori mute, ~6 mA in mute, quindi **~175 mA** nel caso peggiore;
    e, dallo Schottky del comando del guadagno, VRELAY − V_F(42 mA) ≥ 80 % della
    tensione nominale di bobina, a −5 % e a caldo (`reports/2026-09-25-L36-*`);
  - dare il temporizzatore d'accensione, da combinare con l'interruttore di mute
    (ADR-028): da L35 è il contratto scritto accanto a J4 in `preamp_audio.py`,
    con i due comandi sfasati di ADR-045.
...
| ~~Impedenza d'ingresso Singxer SA-1 V2~~ | — | **CHIUSA dall'utente il 2026-09-15**: non pubblicata, e non serve più (4,7 µF; NC-010 chiusa da L17). «Aperti» di `REQUIREMENTS.md` allineato in L38 |
...
| ~~Il LED del trim legge i relè spia K9/K10, non K7/K8~~ | — | **RISPOSTA il 2026-09-15**: l'utente tiene la soluzione attuale, anche per i LED di ADR-030. **Registrata in L38: ADR-033** |
| ~~Soglia su V2: quanto gradino al jack è accettabile~~ | — | **RISPOSTA il 2026-09-15**: ≤ 100 µV di picco 20 Hz–20 kHz, ogni uscita, ogni condizione; cono libero sotto 20 Hz. **Registrata in L38: ADR-032 e V2**, con le risposte di L38 (A gradino + B residuo in mute + C taglio della musica; spegnimento compreso; filtro «indifferente» → 2° ordine; finestra di C 10 ms) |
