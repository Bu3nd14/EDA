...
| **Prossimo lotto** | **L29b** (in corso) — il mute graduale in serie diventa un circuito reale nel sorgente e si misura col metodo di V2 (1 kHz, 20 Hz con rampa da 3 s, 20 kHz), più P7 e lo stato sicuro di ADR-012 col mute in serie. **Diviso**: il caso peggiore di V2 (guadagno e criterio 3 di ADR-030, trim, LSK489, accensione e spegnimento) è **L29c**. Prompt in `NEXT-SESSION.md` |
...
| L29b | **Il mute graduale in serie reale** (L29b diviso il 2026-09-16: il caso peggiore passa a L29c). L'elemento graduale reale in serie nel sorgente (ADR nuova che precisa ADR-012 e ADR-021, tecnica scelta coi numeri), misurato col metodo di V2 su tre uscite e due carichi a **1 kHz**, **20 Hz con rampa da 3 s** e **20 kHz**, col pavimento accanto a ogni C2; **P7** per la posizione in serie; lo stato sicuro di ADR-012 e `check_relay_safe_state.py`. **Non chiude NC-028**: manca il caso peggiore | M/L | NC-028 | da fare |
