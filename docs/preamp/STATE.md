...
| L35 | **Comandi e LED a pannello nel sorgente** (ADR-028). Header di cablaggio al posto dei LED del trim sulla scheda (`trim.py`); comando del guadagno sul rotativo a 3 posizioni: **il cablaggio di SW2 esiste da L36** (4 poli, comando e ponte per bobina, ADR-026 provata dal 2e), resta portarlo a pannello coi LED del guadagno D7–D9; interruttore di mute combinato col temporizzatore d'accensione; LED rosso di mute, letto da un contatto che dica lo stato; `check_relay_safe_state.py` esteso. **Dopo L29 e L36**: il comando del guadagno si cabla una volta sola | S/M | **NC-032**, parte di NC-028 (ADR-045) | da fare — **prossimo**. **Primo punto ADR-045**: K6 su `PERMIT_CMD`, sfasato dopo i relè del jack. Mandato in `NEXT-SESSION.md` |
...
**Aggiornato il 2026-09-22.** Il prossimo è **L39, la Fase 4** (riga L39 della tabella dei lotti, prompt in `NEXT-SESSION.md`); poi **L29c**, il caso peggiore di V2 col mute reale (bozza del mandato: `data/2026-09-22/L29b2/bozza_prompt_L29c.md`). Quello che segue è il testo scritto per L29, tenuto come storico.
...
- **L'alimentatore**, quando arriva, parte da **ADR-020** e da NC-029. Deve
  ...
  - alimentare le bobine col budget di L36: **168,8 mA** a 5 V a +10 dB, fuori
    mute e in mute (72,8 mA a 12 V, 36,8 mA a 24 V), più ~4 mA di LED in mute;
