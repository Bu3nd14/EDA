...
| Suite | `run_tests.sh` **10 passed / 0 failed** a fine L30 (2026-09-26), eseguita dal worktree; `validate_models.py` 48/48 |
...
| L41 | **Il circuito dell'alimentatore** (NC-036, P9, ADR-046; P1–P4). In `circuits/`: trasformatore, raddrizzatore e regolatori ±15 V con la tenuta (≥ 1500 µF effettivi per rail), `VRELAY` con la sua tenuta (≥ 25 ms), il relè di rete con l'interruttore morbido, il sorvegliante (\|13,5 V\|, entro 1 ms, attivo-per-la-musica), il temporizzatore (mute, Δ, LDR di J3, i 13 ms di ADR-027, SW3). Poi il banco di L30 sul circuito vero, col metodo di V2: spegnimento morbido sotto V2; guasto sotto l'obiettivo di 2 mV; controfattuali che falliscono. P2 (sicurezza di rete) parte qui. Probabilmente si divide | L | **NC-036** | da fare — **prossimo**. Mandato in `NEXT-SESSION.md` |
