...
Uso:  /usr/bin/python3 build.py OUTDIR   ->  OUTDIR/ldr1k.cir, poi  ngspice -b ldr1k.cir
...
- la profondita' d: una sola variabile 0 -> 1 in TD secondi dall'inserzione, e al
  ...
  compresa). Serie: 20 mA -> 0 per d in [0, 0.1], lineare. Derivazione: 0 per
  d < 0.35, poi 0,2 uA -> 20 mA log-lineare per d in [0.35, 1]. Fra 0.1 e 0.35 la
  serie si spegne da sola (lenta per natura) prima che la derivazione scenda;
- il rele' al jack: e' il contatto netto BJK* del blocco (col rimbalzo), chiuso a
  TI + TD + 50 ms (profondita' completa, con isteresi) e aperto a TR, prima che la
  derivazione si spenga. Nella corsa "norele" non si chiude mai: la differenza con
...
TD = 3.0          # la corsa della profondita' (ADR-036: 3 s)
ION = 20e-3       # corrente del LED acceso
IMIN = 0.2e-6     # sotto: buio per la curva B (genera_modello.py: buio da 0,19 uA)
L += [
...
L += [riga("ev", "ev", "ldr_td3", A, TI, TR, TF, 10e-6, "evento", "sempre", "mai", TD + 0.1)]
...
# inversione a meta': il rilascio comincia a d = 0,6, a meta' della rampa della derivazione
TINV = TI + 0.6 * TD
L += corsa("inv", A, TI, TINV, 1000, 2000, 8.6, stati=True)
L += [riga("inv", "inv", "inversione_d06", A, TI, TINV, 8.6, 10e-6, "evento", "sempre", "mai", 0.6 * TD)]
...
L += [riga("lzev", "lzev", "ldr_td3", 0, TI, TR, TF, 10e-6, "evento", "lzsempre", "lzmai", TD + 0.1)]
